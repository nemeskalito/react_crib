	...
	return (
		<>
			...
			<div className='impoBlock'>
				...
				<p>
					Когда ты пишешь код на React, TypeScript, Sass, браузер не понимает
					эти файлы напрямую. Сборщик берёт твой код, трансформирует его (JSX →
					JS, TS → JS, SCSS → CSS), объединяет в оптимизированные файлы и отдаёт
					браузеру.
				</p>
				...
				<ul>
					...
					<span>
						Два самых популярных инструмента для этого – Webpack и Vite. Давай
						разберём их.
					</span>
				</ul>
			</div>
			<div className='impoBlock'>
				...
				<ul>
					...
					<li>
						- Строит граф зависимостей (какие файлы друг от друга зависят).
					</li>
					<li>- Собирает всё в один или несколько файлов (bundle.js).</li>
					<li>- Запускает сервер, обновляя страницу при каждом изменении.</li>
				</ul>
				<span>
					<strong>Проблема</strong>: Webpack сначала собирает весь проект, а
					потом раздаёт его браузеру. Это долго.
				</span>
			</div>
			<div className='impoBlock'>
				...
				<ul>
					...
					<li>
						- Не собирает весь проект заранее – он загружает только то, что
						нужно прямо сейчас.
					</li>
				...
				<table>
					<tbody>
						...
						<tr>
							...
							<td>
								Подходит для быстрого старта, средних и небольших проектов.
							</td>
						</tr>
						<tr>
						...
						<tr>
							...
							<td>
								Обновляет только изменённые части (Hot Module Replacement).
							</td>
						</tr>
						<tr>
							<td>
								Гибкость: можно подключать плагины, изменять конфигурацию.
							</td>
